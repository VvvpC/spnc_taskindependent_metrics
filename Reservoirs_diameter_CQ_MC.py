...
if __name__ == "__main__":
    ...
    # print("\nComparison Summary:")
    # print("Beta_prime | Gamma(calc) | MC(single) | MC(coupled) | KR(single) | KR(coupled) | GR(single) | GR(coupled)")
    # print("-" * 100)
    # for i, beta in enumerate(beta_prime_range):
    #     gamma_calc = calculate_gamma_from_beta_prime(beta)
    #     print(f"{beta:8.1f} | {gamma_calc:10.6f} | {results_single['MC'][i]:9.3f} | {results_coupled['MC'][i]:10.3f} | "
    #           f"{results_single['KR'][i]:9d} | {results_coupled['KR'][i]:10d} | "
    #           f"{results_single['GR'][i]:9d} | {results_coupled['GR'][i]:10d}") 